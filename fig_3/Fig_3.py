...
sns.pointplot(x="number of sensors", y="logloss", hue="Classifier", data=results_df[results_df.participant=="Able-bodied"], 
              estimator=estimator, ax=ax1, dodge=dodge, n_boot=n_boot,
              hue_order = hue_order, capsize=capsize, errwidth=errwidth, ci=ci, markers = markers, 
...
ax1.legend(loc=2, ncol=2, frameon=True, edgecolor='.3', bbox_to_anchor=(0., 1.03))
...
sns.pointplot(x="number of sensors", y="logloss", hue="Classifier", data=results_df[results_df.participant=="Amputee"], 
...
sns.boxplot(data=results_df[(results_df["number of sensors"]==2) & (results_df["participant"]=="Able-bodied")], x='Classifier', y='logloss', 
            ax = ax3, width=box_width, linewidth=box_linewidth, palette=palette, showfliers=box_showfliers)
sns.swarmplot(data=results_df[(results_df["number of sensors"]==2) & (results_df["participant"]=="Able-bodied")], x='Classifier', y='logloss', 
            ax = ax3, size=swarm_size, color=swarm_color)
ax4 = fig.add_subplot(324, sharey=ax3)
sns.boxplot(data=results_df[(results_df["number of sensors"]==2) & (results_df["participant"]=="Amputee")], x='Classifier', y='logloss', 
            ax = ax4, width=box_width, linewidth=box_linewidth, palette=palette, showfliers=box_showfliers)
sns.swarmplot(data=results_df[(results_df["number of sensors"]==2) & (results_df["participant"]=="Amputee")], x='Classifier', y='logloss', 
...
for ax, cm_tmp, cbar_ax in zip([ax5, ax6], [cm_ab_mean, cm_amp_mean], [cbar_axis, None]):
    cm_tmp_norm =  cm_tmp/cm_tmp.sum(axis=1)[:, np.newaxis]
    sns.heatmap(100*cm_tmp_norm,  cmap=cmap, vmin=vmin, vmax=vmax, linewidths=linewidths, 
                ax = ax, cbar=True if cbar_ax else False, cbar_ax=cbar_axis, cbar_kws={"orientation": "horizontal"},
                annot=True, fmt = "3.1f",
...
plt.savefig('Fig_3.pdf', dpi=600, bbox_inches='tight',transparent=False, pad_inches=0)